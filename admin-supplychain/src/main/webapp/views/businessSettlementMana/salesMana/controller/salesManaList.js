(function (angular) {
    function salesManaCtrl($scope, DTOptionsBuilder, salesManaService, commonService, DTColumnBuilder, common, SweetAlertX, $filter, $uibModal) {
        ...
        $scope.dtColumns = [
            ...
            DTColumnBuilder.newColumn('productName').withOption('width', 120).withOption('ellipsis', true).withTitle('产品名称').renderWith(function (e, dt, node, config) {
                return node.productInfo.name;
            }),
            DTColumnBuilder.newColumn('specification').withOption('width', 140).withOption('ellipsis', true).withTitle('产品规格').renderWith(function (e, dt, node, config) {
                return node.productInfo.specification;
            }),
            DTColumnBuilder.newColumn('type').withOption('width', 120).withOption('ellipsis', true).withTitle('设备类型').renderWith(function (e, dt, node, config) {
                for (var i in $scope.typeIdList) {
                    if ($scope.typeIdList[i].number == node.productInfo.type) {
                        return $scope.typeIdList[i].text
                    }
                }
            }),
            DTColumnBuilder.newColumn('orderNumber').withOption('width', 200).withOption('ellipsis', true).withTitle('关联订单'),
        ...
        $scope.detail = function (item) {
            salesManaService.getSalesInfoByid({id:item.id}, function (data) {
                var modalInstance = $uibModal
                    .open({
                        templateUrl: omssupplychainAdminHost + 'views/businessSettlementMana/salesMana/view/details.html',
    ...
    angular.module(businessSettlementMana).controller('salesManaCtrl', salesManaCtrl);
})(angular);

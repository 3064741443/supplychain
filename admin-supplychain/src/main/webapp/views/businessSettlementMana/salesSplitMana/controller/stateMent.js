(function (angular) {
    function stateMentDefine(common, $scope, salesManaService, param, DTOptionsBuilder, DTColumnBuilder, $uibModalInstance, SweetAlertX) {
        ...
        for(var i=0;i<$scope.productArr.length;i++){
            for(var j=0;j<$scope.productArr[i].productHistoryPriceList.length;j++){
                $scope.merchant.salesSummarizingMaterialDetailList.push($scope.productArr[i].productHistoryPriceList[j])
            }
        }
            ...
            for (var i = 0; i < $scope.productArr.length; i++) {
                for (var j = 0; j < $scope.productArr[i].productHistoryPriceList.length; j++) {
                    for (var x = 0; x < $scope.materialInfo.length; x++) {
                        if ($scope.productArr[i].productHistoryPriceList[j].materialCode == $scope.materialInfo[x].materialCode) {
                            $scope.productArr[i].productHistoryPriceList[j].materialName = $scope.materialInfo[x].materialName;
                            $scope.productArr[i].productHistoryPriceList[j].price = $scope.materialInfo[x].price;
                        }
                    }
                }
            }
        ...
        $scope.priceCheck = function(index){
            ...
            if(key == 0){
                ...
                for(var i=0;i<$scope.productArr.length;i++) {// 第一层数据
                    for (var j = 0; j < $scope.productArr[i].productHistoryPriceList.length; j++) { // 第二层数据
                        $scope.productArr[i].productHistoryPriceList[j].materialPrice = $scope.stateMentDetailList[sum].price;
                        sum++;
                    }
                }
            }

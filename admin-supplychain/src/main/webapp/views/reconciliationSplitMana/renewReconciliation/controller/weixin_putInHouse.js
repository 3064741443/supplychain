(function(angular) {
    function renewWeixinPutInHouseDefine($scope, renewService, comService, $uibModalInstance, SweetAlertX,FileUploader,param) {
        ...
        $scope.downloadTemplate = function() {
            var path = omssupplychainAdminHost+"statementSellInfo/statementSellRenewDownloadTemplate";
            location.href =path;
        };

        var uploaderDeviceList = $scope.uploaderWeixinList = new FileUploader({
            url : omssupplychainAdminHost+"statementSellInfo/statementSellRenewImportDataCheck",
        ...
        $scope.uploaderWeixinList.onBeforeUploadItem = function (fileItem) {
            $scope.importLoadShow = true;
        };
        // 上传控件：回调响应：
        $scope.uploaderWeixinList.onCompleteItem = function(item, response, status, headers) {
            $scope.importLoadShow = false;
        ...
        $scope.importWeixinList = function() {
            if ($scope.outerForm.$valid) {
                if(importStatementCollection=="" || importStatementCollection == null || importStatementCollection == "null"){
                    ...
                    // if($scope.formData.saleGroup == undefined){
                    ...
                    var Data = {
                    		...
                    		listStatementSellRenewImport:importStatementCollection
                    };
                    
                    renewService.statementSellRenewImport(Data, function(data) {
    ...
    angular.module('reconciliationSplitMana').controller('renewWeixinPutInHouseDefine', renewWeixinPutInHouseDefine);
})(angular);
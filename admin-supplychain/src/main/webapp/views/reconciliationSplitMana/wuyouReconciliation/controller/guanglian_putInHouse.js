(function(angular) {
    function guanglianwuyouPutInHouseDefine($scope, wuyouService, comService, $uibModalInstance, SweetAlertX,FileUploader,param) {
        ...
        $scope.downloadTemplate = function() {
            var path = omssupplychainAdminHost+"statementSellInfo/statementSellGlwyDownloadTemplate";
            location.href =path;
        };

        var uploaderDeviceList = $scope.uploaderGlwyList = new FileUploader({
            url : omssupplychainAdminHost+"statementSellInfo/statementSellGlwyImportDataCheck",
        ...
        $scope.uploaderGlwyList.onBeforeUploadItem = function (fileItem) {
            $scope.importLoadShow = true;
        };
        // 上传控件：回调响应：
        $scope.uploaderGlwyList.onCompleteItem = function(item, response, status, headers) {
            $scope.importLoadShow = false;
        ...
        $scope.importGlwyList = function(status) {
            if ($scope.outerForm.$valid) {
                if(importStatementCollection=="" || importStatementCollection == null || importStatementCollection == "null"){
                    ...
                    var Data = {
                        ...
                        listStatementSellGlwyImport:importStatementCollection
                    };
                    
                    wuyouService.statementSellGlwyImport(Data, function(data) {
    ...
    angular.module('reconciliationSplitMana').controller('guanglianwuyouPutInHouseDefine', guanglianwuyouPutInHouseDefine);
})(angular);
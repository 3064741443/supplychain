(function (angular) {
    function merchantOrderBusCtrl($scope, DTOptionsBuilder, MerchantOrderService, DTColumnBuilder, common,commonService, $filter, $uibModal,SweetAlertX) {
        ...
        $scope.checkorder = function (item) {
            MerchantOrderService.getMerchantOrderInfo({orderNumber: item.orderNumber}, function (data) {
                var modalInstance = $uibModal.open({
                    templateUrl: omssupplychainAdminHost + 'views/supplyChainSalesMana/merchantOrderBussiness/view/checkOrder.html',
    ...
    angular.module(supplyChainSalesMana).controller('merchantOrderBusCtrl', merchantOrderBusCtrl);
})(angular);

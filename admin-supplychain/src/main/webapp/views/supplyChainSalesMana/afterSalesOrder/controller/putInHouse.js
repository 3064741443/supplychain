(function(angular) {
    function importSnDefine($scope,afterOrderService, $uibModalInstance, FileUploader) {
        $scope.importIsShow=false;
        $scope.importLoadShow=false;
        var importSn="";
        var importFileUrl="";
        $scope.downloadTemplate = function() {
            var path = omssupplychainAdminHost+"afterSales/SndownloadTemplate";
            location.href =path;
        };

        var uploaderSnList = $scope.uploaderSnList = new FileUploader({
            url : omssupplychainAdminHost+"afterSales/snImportData",
...
/*                    myAfterService.afterSnImport({strParam:obj}, function(data) {
    ...
    angular.module(supplyChainSalesMana).controller('importSnDefine', importSnDefine);
})(angular);
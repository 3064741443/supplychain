...
@Service
public class JXCDeviceTypeRedisManager {
	...
	*/
	public String getDeviceTypeById(Integer id)
	{
		...
		{
			strValue = redisClient.opsForValue().get(RedisEnum.JXCREDIS_DEVICE_TYPE_INFO.getValue() + id);
		}
	 ...
	 * @throws
	 */
	 public void setDeviceTypeById(JXCMTDeviceType deviceType)
	 {
		 ...
		 {
			 redisClient.opsForValue().set(RedisEnum.JXCREDIS_DEVICE_TYPE_INFO.getValue() + deviceType.getId(), deviceType.getName());
		 }
	 ...
	 */
	 public void delDeviceType(Integer id)
	 {
		 ...
		 if(!StringUtils.isEmpty(id))
		 {
			 redisClient.delete(RedisEnum.JXCREDIS_DEVICE_TYPE_INFO.getValue() + id);
		 }

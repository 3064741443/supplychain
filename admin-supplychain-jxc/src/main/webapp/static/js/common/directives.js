    ...
    return {
        link: function(scope, element) {
            var listener = function(event, toState, toParams, fromState, fromParams) {
                ...
                if (toState.data && toState.data.pageTitle) title = '广联运营平台流量卡管理 | ' + toState.data.pageTitle;
                $timeout(function() {
                    element.text(title);
                });
            };
...
function iboxTools($timeout) {
    return {
        restrict: 'A',
        scope: true,
        templateUrl: 'biz/common/ibox_tools.html',
...
function treeSelect() {
    return {
        ...
        template: ' <div class="tree-select relative">' +
...
function dualTablebox(DTOptionsBuilder, DTColumnBuilder, $compile, $filter, $q) {
    return {
        ...
        replace: true,
        transclude: true,
        templateUrl: 'biz/common/dualTablebox.html',
        controller: function($scope) {
            var originalData = [],
                selectingList = [],
                selectedList = [],
            ...
            function init(dtOption) {
                ...
                $scope.selecting_dtOptions = DTOptionsBuilder
                    ...
                    .withOption('bFilter', false) // 禁用搜索框
                    .withButtons([])
                    ...
                    .withOption('scrollY', "350px") // tbody height
            ...
            function parseTable(data) {
                ...
                data.columns.forEach(function(item) {
                    ...
                    item.width && dtColumn.withOption('width', item.width).renderWith(function(data, type, full, oSettings) {
                        return '<div title="' + full.data[item.prop] + '" class="text-ellipsis" style="width:' + (item.width - 17) + 'px;">' + full.data[item.prop] + '<div>';
                    });
...
/**
 * [daterangepickerExtend 时间范围快捷选择]
 */
function daterangepickerExtend() {
    var getRangeDate = function(range) {
        var start, end;
        if (range < 0) {
            start = moment().add(range, 'd').startOf('day');
            end = moment().add(-1, 'd').endOf('day');
        } else if (range > 0) {
            start = moment().add(1, 'd').startOf('day');
            end = moment().add(range, 'd').endOf('day');
        } else {
    ...
    return {
        ...
        template: ' <div class="row">' +
            '     <div class="col-sm-7 no-padding range-group">' +
            '         <button class="btn btn-link pull-left" data-range="0" type="button">今天</button>' +
            '         <button class="btn btn-link pull-left" data-range="-1" type="button">昨天</button>' +
            '         <button class="btn btn-link pull-left" data-range="-7" type="button">最近7天</button>' +
            '         <button class="btn btn-link pull-left" data-range="-30" type="button">最近30天</button>' +
            '         <button class="btn btn-link pull-left" data-range="-60" type="button">最近60天</button>' +
            '     </div>' +
            '     <div class="col-sm-5">' +
            '         <input date-range-picker class="form-control date-picker" placeholder="自定义" type="text" ng-model="ngModel" options="{eventHandlers: {\'apply.daterangepicker\': reset}}" />' +
            '     </div>' +
            ' </div>',
        link: function(scope, element) {
            var activeButton;
            element.find('.range-group').on('click', 'button', function(e) {
                scope.$parent.safeApply(function() {
                    var curButton = $(e.target),
                        onActive = curButton.hasClass('active'),
                        range;
                    ...
                    if (onActive) {
                        ...
                        curButton.removeClass('btn-link').addClass('active btn-primary');
                        range = getRangeDate(curButton.data('range'));
                    }

                    scope.ngModel = scope.ngModel || {};
                    angular.extend(scope.ngModel, range);
                    activeButton = curButton;
                });
            });
            var reset = function() {
                activeButton && activeButton.addClass('btn-link').removeClass('active btn-primary');
            };
            var resetWatch = scope.$watch('ngModel', function(newValue) {
                if (!newValue) {
                    reset();
                }
            });
...
/**
 * [clearable 清除 input 内容, 'X' icon]
 * @return {[type]} [description]
 */
function clearable ($parse) {
    var dom = '<div class="clearable"></div>';
    var defaults = {
        width: 20,
        height: 20
    };
        ...
        link: function(scope, element, attr) {
            ...
            function toggle(targetFlag) {
                if (targetFlag === true) {
                    ...
                    $dom.toggle();
                    targetFlag = !toggleFlag;
                }

                toggleFlag = targetFlag;
                if (toggleFlag) {
                    p = element.position();
                    width = element.outerWidth();
                    ml = parseFloat(element.css('margin-left'));
                    element.addClass('clearable-fixed');
                } else {
                    element.removeClass('clearable-fixed');
                }
                
            }

            element.on('keyup.clearable focus.clearable paste.clearable', function() {
                if (!!$(this).val().trim() !== toggleFlag) {
                    toggle();
                }
            }).on('blur.clearable',function(e){
                toggle(false);             
            });
            ...
            var $dom = $(dom).css({
                    left: p.left + width + ml - setting.width - 5 + 'px',
                    // top: p.top + mt + (height - setting.height) / 2 - 3 + 'px'
                })
                .on('mousedown.clearable',function() {
...
/**
 * [uploadShell 上传控件的壳（样式）]
 */
function uploadShell($parse) {
    var temp = '<div class="row">' +
        '    <div class="col-xs-9">' +
        '        <input type="text" class="form-control">' +
        '    </div>' +
        '    <div class="col-xs-3">' +
        '        <button class="btn btn-primary btn-outline">选择文件</button>' +
        '    </div>' +
        '</div>';
    return {
        link: function(scope, element, attr) {
            ...
            element.on('change', function() {
                ...
                $dom.find('input').val(filename);
                setter(scope, filename);
            });
            element.addClass('hidden').after($dom);
        }
    };
}

/**
 * [checkLength 实时监听textarea字数]
 */
function checkLength() {
    var total = 50;
    var lengthHtml = '<span class="pull-right m-xs"><span class="real-length text-danger">0</span>/<span class="max-length">50</span>';
    return {
        restrict: 'A',
        link: function(scope, element, attr) {
            var $dom = $(lengthHtml);
            var $realLengh = $dom.find('.real-length');
            $dom.find('.max-length').text(attr.checkLength || attr.maxlength || total);

            $(element).after($dom).on('keyup paste', function() {
                var self = this;
                setTimeout(function() {
                    $realLengh.text(self.value.length);
                }, 0);
            });
        }
    };
...
/**
 * [advance 解决absolute造成的高度塌陷]
 */
function advance() {
    return {
        restrict: 'C',
        link: function(scope, element, attr) {
            var $wrapper = element.parents('.advance-wrapper');

            var displayWatch = scope.$watch(function() {
                setTimeout(function() {
                    var newValue = element.css('display');
                    if (newValue !== 'none') {
                        $wrapper.css('min-height', element.parent().height() + element.height() + 120);
                    } else {
...
angular
    ...
    .directive('daterangepickerExtend', daterangepickerExtend)
    .directive('checkLength', checkLength)
    .directive('uploadShell', uploadShell)
    .directive('clearable', clearable)
    .directive('advance', advance);

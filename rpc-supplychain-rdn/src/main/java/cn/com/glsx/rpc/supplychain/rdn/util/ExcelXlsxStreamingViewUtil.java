...
import cn.com.glsx.rpc.supplychain.rdn.service.BsLogisticsService;
...
@Component
public class ExcelXlsxStreamingViewUtil {
	
	private static final Logger logger = LoggerFactory.getLogger(ExcelXlsxStreamingViewUtil.class);
	 ...
	 * @throws Exception
	 */
	private XSSFWorkbook initWorkbook(String excelTemplateFilePath)
			throws Exception {
		// 读取导出模板文件
	...
	private Sheet cloneSheet(XSSFWorkbook workbook, int sheetIndex,
			String sheetName) {
	...
	private CellStyle customeCellStyle(XSSFWorkbook workbook) {
		...
		return cellStyle;// Font cellFont = workbook.createFont();
	...
	private boolean isMoneyFomatCell(String propertyName) {
	...
	private boolean isNumFormatCell(String propertyName) {
		if (propertyName.equals("bsTotal")) {
			return true;
		}
		if (propertyName.equals("bsCheckQuantity")) {
			return true;
		}
		if (propertyName.equals("bsSendQuantity")) {
			return true;
		}
		if (propertyName.equals("bsOweQuantity")) {
			return true;
		}
		if (propertyName.equals("shipmentsQuantity")) {
			return true;
		}
		return false;
	}
	...
	public void buildExcelDocument(Map<String, Object> model,
			Workbook workbook, FileOutputStream outputStream) throws Exception {
		String fileName = model.getOrDefault(EXCEL_FILE_NAME_KEY,
				"my-xls-file.xlsx").toString();
		String excelTemplateFilePath = model.getOrDefault(
				EXCEL_TEMPLATE_FILE_PATH_KEY,
				"/templates/templateMerchantOrder.xlsx").toString();
		String sheetName = model.getOrDefault(EXCEL_SHEET_NAME_KEY, "sheet")
				.toString();
		...
		if (objs != null && objs.size() > 0) {
			for (int i = 0; i < objs.size(); i++) {
				...
				if (rowIndex == 0 || rowIndex == 65535) {
					sheet = cloneSheet(xssfWorkbook, sheetIndex, sheetName
							+ (sheetIndex == 1 ? "" : (sheetIndex - 1)));
	...
	//				newCell.setCellType(HSSFCell.CELL_TYPE_STRING);
					String cellValue = "";
					cellValue = getFieldValueByFieldName(
							propertyColumns[columnIndex], vo);
					if (cellValue == null) {
						cellValue = "";
					}

					// 单元格格式设置
					if (this.isNumFormatCell(propertyColumns[columnIndex])) {
						DataFormat format = xssfWorkbook.createDataFormat();
						cellStyle.setDataFormat(format.getFormat("#,#0"));
						newCell.setCellStyle(cellStyle);
						if (!RdnUtils.parseInt(cellValue)) {
							cellValue = "0";
						}
						newCell.setCellValue(Integer.parseInt(cellValue));
					} else if (this
							.isMoneyFomatCell(propertyColumns[columnIndex])) {
						DataFormat format = xssfWorkbook.createDataFormat();
						cellStyle.setDataFormat(format.getFormat("0.00"));
						newCell.setCellStyle(cellStyle);
						if (!RdnUtils.parseDouble(cellValue)) {
							cellValue = "0";
						}
						String strValue = df.format(Double
								.parseDouble(cellValue));
						newCell.setCellValue(Double.parseDouble(strValue));
					} else {
						DataFormat format = xssfWorkbook.createDataFormat();
	...
	/*
	 * @Override public void buildExcelDocument(Map<String, Object> model,
	 * Workbook workbook, HttpServletRequest request, HttpServletResponse
	 * response) throws Exception { String fileName =
	 * model.getOrDefault(EXCEL_FILE_NAME_KEY,"my-xls-file.xlsx").toString();
	 * response.setContentType("application/vnd.ms-excel"); //change the file
	 * name; response.setHeader("Content-disposition", "attachment;filename=" +
	 * URLEncoder.encode(fileName, "utf-8"));
	 * 
	 * OutputStream ouputStream = response.getOutputStream();
	 * 
	 * workbook.write(ouputStream); ouputStream.flush(); ouputStream.close(); }
	 */

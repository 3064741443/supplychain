(function (angular) {
    function placeOrderOKDefine($scope, param, myOrderService, common, $uibModalInstance, SweetAlertX) {
        if (param == null) {
            $scope.orderOKData= {};
        } else {
        ...
        $scope.ok = function () {
            ...
            if ($scope.add_Order_form.$valid) {
                ...
                myOrderService.addSplitMyOrder(obj, function (data) {
                    if (data.returnCode == '0') {
                        SweetAlertX.alert('', '提交成功', 'success');
                        $uibModalInstance.close(data);
                    } else {
                        if (data.message) {
                            if(data.message == 'houtai'){
                                SweetAlertX.alert(data.message, '后台版本升级请清除缓存', 'error');
                            }else{
                ...
                $scope.add_Order_form.submitted = true;
            }
        };
    ...
    angular.module(dealerManage).controller('placeOrderOKDefine', placeOrderOKDefine);
})(angular);
(function (angular) {
    function shipmentDetails($scope, param, DTOptionsBuilder, myStockService, DTColumnBuilder, common, $uibModal, $uibModalInstance, $filter, SweetAlertX) {
    	var materialCode = param.materialCode;
        //加载列表数据
        $scope.dtInstance = {
            serverData: function (param) {
            	param.materialCode = materialCode;
                return myStockService.pageMerchantStockSell(param, function (response) {
    ...
    angular.module(dealerManage).controller('shipmentDetails', shipmentDetails);
})(angular);

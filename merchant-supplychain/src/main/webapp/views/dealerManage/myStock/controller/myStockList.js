(function (angular) {
    function myStockCtrl($scope, DTOptionsBuilder, myStockService, DTColumnBuilder, common, commonService, $filter, $uibModal,SweetAlertX) {

        //加载列表数据
        $scope.dtInstance = {
            serverData: function (param) {
                return myStockService.listMerchantStock(param, function (response) {
                    common.dtDataMapConfig(response);
                });
            }
        };
        
        myStockService.statMerchantStocks(function(result){
        ...
        $scope.search = function () {
            ...
            myStockService.statMerchantStocks(formData, function(result){
            	if(null == result.data){
        ...
        $scope.exportExcel = function () {
            ...
            var url = omssupplychainAdminHost + "myStock/exportMerchantStockList?e=1";
            if (form.materialCode && form.materialCode != '') {
                url +="&materialCode=" + form.materialCode;
                url +="&materialName=" + form.materialCode;
            }
		...
		$scope.detail = function(item) {
			...
			var modalInstance = $uibModal.open({
				templateUrl: omssupplychainAdminHost + 'views/dealerManage/myStock/view/shipmentDetails.html',
    ...
    angular.module(dealerManage).controller('myStockCtrl', myStockCtrl);
})(angular);

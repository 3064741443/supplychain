...
public class AttribInfoRedisManager {
	...
	*/
	public AttribInfo getAttribInfoById(Integer id)
	{
		...
		String redisString = "";
		redisString=redisClient.opsForValue().get(RedisEnum.JST_REDIS_ATTRI_INFO.getValue() + id);
	...
	*/
	public void setAttribInfoById(AttribInfo attribInfo)
	{
		...
		jsonObject.put("value", attribInfo.getValue());
		String redisString = jsonObject.toString();
		redisClient.opsForValue().set(RedisEnum.JST_REDIS_ATTRI_INFO.getValue() + attribInfo.getId(), redisString);
